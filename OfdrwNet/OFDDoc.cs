...
namespace OfdrwNet
{
    ...
    public class OfdWriter : IDisposable, IOfdDocWriter
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(strategy)) _imageOrderingStrategy = strategy;
            return this;
        }
        ...
        public OfdWriter AddRawImage(string format, double x, double y, double width, double height, byte[] data, int page = 1, double[]? ctm = null)
        {
            ...
            _rawImages.Add(img);
            if (first) { _imageHashFirst[hash] = img; _logger?.LogDebug("[OFDDoc][Image] Page={Page} NewImage Hash={Hash} {W}x{H}mm Format={Fmt} Z={Z}", img.Page, hash[..Math.Min(12, hash.Length)], img.Width.ToString("0.##"), img.Height.ToString("0.##"), img.Format, img.Z); }
            else { _logger?.LogDebug("[OFDDoc][Image] Page={Page} DupImage Hash={Hash} Z={Z}", img.Page, hash[..Math.Min(12, hash.Length)], img.Z); }
            return this;
        }
        ...
        public async Task CloseAsync()
        {
            ...
            try
            {
                ...
                if (_virtualPageList.Count == 0) _virtualPageList.Add(new OfdrwNet.Models.VirtualPage(_pageLayout));
        ...
        private async Task CreateOfdStructureAsync(string baseDir)
        {
            ...
            try
            {
                var docInfo = new OfdrwNet.Core.BasicStructure.Ofd.DocInfo.CtDocInfo()
                    .RandomDocID(); // 仅设置必需的 DocID，可扩展其它元数据

                var docBody = new OfdrwNet.Core.BasicStructure.Ofd.DocBody()
                    .SetDocInfo(docInfo)
                    .SetDocRoot(new OfdrwNet.Core.BasicType.StLoc("Doc/Document.xml"));

                var ofd = new OfdrwNet.Core.BasicStructure.Ofd.OFD(docBody);
                // 规范要求 Version 1.2，目前 Core 模型常量也是 1.2；若需兼容旧值可提供配置点
                // 这里保持模型默认属性，不再硬编码 1.1
            ...
            catch (Exception ex)
            {
                ...
                // 回退最小结构以保障后续流程继续（避免整体失败）
                var fallback = "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
                               "<ofd:OFD xmlns:ofd=\"http://www.ofdspec.org/2016\" DocType=\"OFD\" Version=\"1.2\">\n" +
                               "  <ofd:DocBody>\n" +
                               "    <ofd:DocInfo><ofd:DocID>" + Guid.NewGuid().ToString("N") + "</ofd:DocID></ofd:DocInfo>\n" +
                               "    <ofd:DocRoot>Doc/Document.xml</ofd:DocRoot>\n" +
                               "  </ofd:DocBody>\n" +
                               "</ofd:OFD>";
                await FileUtil.WriteTextFileUtf8LfAsync(Path.Combine(baseDir, "OFD.xml"), fallback);
            }
        ...
        private async Task<string> GenerateDocumentXmlAsync()
        {
            ...
            {
                var pid = GetNextID();
                pagesElement.AddPage(new OfdrwNet.Core.BasicStructure.PageTree.Page(pid, $"Pages/Page_{pg.PageNumber}/Content.xml"));
            }

            // 注意：上面分配页面ID时调用了 GetNextID 可能会提升 _maxUnitID，需要更新 CommonData 的 MaxUnitID
            if (_maxUnitID < 0) _maxUnitID = 0; // 防御性
            // 重新覆盖最新 MaxUnitID
        ...
        private async Task GeneratePageFilesAsync(string pagesDir)
        {
            if (_fontMap.Count == 0)
            {
                ...
                foreach (var kv in fontRegistry.Build(_externalFontFiles, _streamQueue)) _fontMap[kv.Key] = kv.Value;
            }
            if (_pageGroups == null)
            {
                var pgBuilder = new OfdrwNet.Services.PageGroupBuilder(_logger);
            ...
            {
                await writer.WritePageAsync(pagesDir, pg.PageNumber, pg.Items, _rawImages, _rawPaths, _fontMap, _imageOrderingStrategy, GetNextID, _pageLayout.Width, _pageLayout.Height);
            }
        }
