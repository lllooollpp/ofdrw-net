...
/// 3. 选项：去除 Alpha（平铺背景）
/// </summary>
public class OfdImagePreprocessor
{
    ...
    public OfdImagePreprocessor(bool flattenAlpha = false, string flattenColorHex = "#FFFFFF")
    {
    ...
    /// 预处理结果
    /// </summary>
    public readonly record struct PreprocessResult(byte[] Data, string FormatExt /* 不含点，如 png */);
    ...
    public PreprocessResult Process(byte[] rawBytes)
    {
        try
        {
            using var msLoad = new MemoryStream(rawBytes, writable: false);
            IImageFormat? detectedFormat = SixImage.DetectFormat(msLoad);
            msLoad.Position = 0;
            using var image = SixImage.Load(msLoad); // 原格式加载（保持所有帧）
            ...
            if (!needConvert && !_flattenAlpha)
            {
                ...
                return new PreprocessResult(rawBytes, targetExt);
            }
        ...
        {
            // 解析失败，返回原始
            return new PreprocessResult(rawBytes, "dat");
        }
    }

    private static bool HasAlpha(SixLabors.ImageSharp.Image image)
    {
    ...
    private static PreprocessResult EncodePng(SixLabors.ImageSharp.Image<Rgba32> img)
    {
        ...
        img.Save(ms, encoder);
        return new PreprocessResult(ms.ToArray(), "png");
    }
}
