...
namespace OfdrwNet.Converter.Refactor
{
    ...
    {
        public async Task ExtractAsync(PdfDocument pdfDoc, IOfdDocWriter ofd, PdfToOfdOptions options, ILogger? logger, System.Threading.CancellationToken token)
        {
            ...
            for (int p = 1; p <= pages; p++)
            {
                ...
                var strat = new SimpleGroupingStrategy(page.GetPageSize(), options);
        ...
        private static List<TextSegment> BuildSegments(IReadOnlyList<TextRenderInfo> infos, PdfToOfdOptions options, ILogger? logger)
        {
        ...
        private static void EmitSegment(TextSegment segment, IOfdDocWriter writer, PdfToOfdOptions options, double pageHeightPt, int pageNumber, ILogger? logger)
        {
            ...
            writer.AddRawTextGlyphRun(fontName, fontSizeMm, originXmm, originYmm, widthMm, heightMm, segment.Text, deltaX, null, null, pageNumber,
                (ctm.Length == 0 ? null : ctm), baselineY: pageHeightMm - baselineYmm, charStarts: charStarts, charAdvances: charAdvances);
        }

    private static GapDecision EvaluateGap(TextRenderInfo left, TextRenderInfo right, RunAnalysis analysis, PdfToOfdOptions options)
        {
        ...
        private static (double[] Ctm, double ScaleX, double ScaleY) BuildNormalizedCtm(TextRenderInfo info, ILogger? logger)
        {
            ...
            double scaleX = Math.Abs(pdfCtm.Get(iText.Kernel.Geom.Matrix.I11));
            double scaleY = Math.Abs(pdfCtm.Get(iText.Kernel.Geom.Matrix.I22));
        ...
        private sealed class RunAnalysis
        {
            ...
            private static bool IsCjkOrEastAsian(char ch)
            {
                ...
                if (code >= 0x2E80 && code <= 0x2EFF) return true; // CJK radicals
                ...
                if (code >= 0xFF66 && code <= 0xFF9F) return true; // Halfwidth Katakana
        ...
        private class SimpleGroupingStrategy : IEventListener
        {
            ...
            public SimpleGroupingStrategy(Rectangle pageSize, PdfToOfdOptions options)
            {
                _pageSize = pageSize;
                _options = options ?? new PdfToOfdOptions();
            }
            ...
            public void EventOccurred(IEventData data, EventType type)
            {
                ...
                if (_current.Count > 0)
                {
                    ...
                    // 计算用于 delta 的参考：使用 end-to-start (上一个字形的 end -> 当前字形的 start)
                    var lastEndX = last.GetBaseline().GetEndPoint().Get(0);
                    var curStartX = ri.GetBaseline().GetStartPoint().Get(0);

                    // 动态阈值：基于字体大小（pt）计算基线/回退阈值，避免固定 6pt 在不同字号下失效
                    ...
                    {
                        // 处理微小负 kerning（负间距），当其绝对值小于选项允许吸收的最大负间距时，将其视作 0
                        // 使用字形尾到下一个字形起点来计算真实间隙，替代原先的 start-to-start 逻辑
                        float delta = (float)(curStartX - lastEndX);
                        if (delta < 0)
                        {
                            double maxNegPt = (double)GeometryUtils.MmToPt(_options?.MaxNegativeKerningAbsorbMm ?? 0.25d);
                            if (Math.Abs(delta) <= maxNegPt)
                            {
                                delta = 0f; // 吸收小的负 kerning
                            }
                        }
                        _deltas.Add(delta);
                    }
